...
def generate_launch_description():
    ...
    pkg_share = FindPackageShare("so101_hardware_interface")
    ...
    robot_description_content = ParameterValue(
        Command(
            [
                ...
                " ",
                PathJoinSubstitution([pkg_share, "config", "so101_ros2_control.urdf.xacro"]),
                " device_port:=",
                device_port,
            ]
        ),
    ...
    controller_manager = Node(
        ...
        parameters=[
            robot_description,
            PathJoinSubstitution([pkg_share, "config", "so101_controllers.yaml"]),
        ],
        output="screen",
    )
